...
| 索引編號 | 日期 | 主題 | 主要檔案 | 狀態 |
|---|---|---|---|---|
| `CR-210` | 2026-08-27 | [正交折線接點緩衝間距擴展與折角貼近優化（提升至 36px 確保直角折線不擠壓接點）](#cr-210) | `orthogonalRouting.ts` | 已驗證 |
...
### <a id="cr-210"></a>CR-210 (2026-08-27) — 正交折線接點緩衝間距擴展與折角貼近優化（提升至 36px 確保直角折線不擠壓接點）

- **使用者需求**：不要讓 90 度的線跟接點太接近，都擠在一起了。
- **排查與修復實作**：
  1. **擴展正交折線緩衝間距 (`orthogonalRouting.ts`)**：
     - 將預設緩衝間距（`margin`）由 20px 擴展至 36px，確保連線從起點接點出發或進入終點接點時，至少有 36px 的筆直延伸距離。
  2. **重構直角轉折幾何計算 (`orthogonalRouting.ts`)**：
     - 重構 `buildOrthogonalPath` 水平/垂直及轉折線型邏輯，當目標接點位於反向或折角空間不足時，自動預留 `margin` 雙向迴轉安全淨空，杜絕 90 度折點緊貼接點把手（Handle）造成的擁擠與視覺重疊。
