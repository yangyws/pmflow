...
### <a id="cr-211"></a>CR-211 (2026-08-28) — 網址導覽狀態同步與 F5 原頁重新整理支援（URL Search Params 雙向同步、保留專案/頁籤/任務/篩選/帳號視圖、消除 F5 誤回首頁）

- **使用者需求**：不要F5 回到首頁 應該是留在該頁 做整個刷新。
- **排查與修復實作**：
  ...
  2. **F5 / 重新整理無縫留存當前畫面 (`App.tsx`)**：
     - 瀏覽器 F5 全頁重新整理時，初始 state 自動優先由 URL 參數解析還原，精準保留使用者所在的專案、檢視頁籤（關聯圖/甘特圖/流程圖/看板/日曆/儀表板等）、開啟中的任務抽屜及側欄篩選大項目，並完整觸發後端資料全域重新拉取刷新。
  3. **上一頁/下一頁歷史導覽與首頁狀態相容 (`App.tsx`)**：
     - 監聽 `popstate` 事件以無縫支援瀏覽器上一頁/下一頁前進後退。
     - 點擊「切換專案」或登出時精準清空網址 Query String 並回退至專案選擇頁；專案清單載入後自動校驗專案有效性。
