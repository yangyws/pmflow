...
| 索引編號 | 日期 | 主題 | 主要檔案 | 狀態 |
|---|---|---|---|---|
| `CR-220` | 2026-09-03 | [專案軟刪除（標記刪除）與超級管理者專案還原管理功能](#cr-220) | `projects.ts`, `auth.ts`, `api.ts`, `ProjectPicker.tsx`, `ProjectSettings.tsx`, `AdminPanel.tsx` | 已驗證 |
...
- **排查與實作**：
  ...
  2. **前端 API 與專案首頁卡片整合 (`api.ts`, `ProjectPicker.tsx`)**：
     - 新增 `DeletedProject` 型別與 `deleteProject`、`deletedProjects`、`restoreProject` 前端 API 方法。
     - 在首頁專案卡片底部，當使用者為專案建立者或超級管理者時呈現 🗑️ 刪除按鈕，並配置二次防誤刪確認彈窗（清楚提示資料封存、僅超級管理者可還原）。
  3. **專案設定危險區域與全站管理員還原面板 (`ProjectSettings.tsx`, `AdminPanel.tsx`)**：
     - 在專案設定頁底部為建立者與超級管理者新增「⚠️ 危險區域：刪除專案」區塊與確認彈窗，刪除後自動返回專案首頁。
