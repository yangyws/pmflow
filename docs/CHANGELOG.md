...
| 索引編號 | 日期 | 主題 | 主要檔案 | 狀態 |
|---|---|---|---|---|
| `CR-101` | 2026-08-09 | [頁籤：新增「未分派」視圖頁籤，快速找出與顯示未被指派負責人之事件與任務](#cr-101) | `App.tsx`, `nav.ts` | 已驗證 |
...
### <a id="cr-101"></a>CR-101 (2026-08-09) — 頁籤：新增「未分派」視圖頁籤，快速找出與顯示未被指派負責人之事件與任務

1. **新增頁籤與文字**：於 `nav.ts` 新增 `unassigned: '未分派'`，並於 `App.tsx` 註冊 `'unassigned'` 視圖 key 至 `VIEWS` 陣列。
2. **自動篩選未分派任務**：當切換至「未分派」頁籤時，透過 `visible.filter(t => !t.assigneeId)` 即時過濾出無負責人之任務與事件，以樹狀清單結構呈現。
3. **完全相容偏好與拖曳**：支援個人瀏覽器頁籤隱藏/顯示偏好與 `dnd-kit` 頁籤左右拖曳排序。
