...
| `CR-208` | 2026-08-26 | [系統流程圖連線自訂變色支援與關聯圖/流程圖過近折點把手自動隱藏（消除相鄰卡片擁擠）](#cr-208) | `SystemFlow.tsx`, `TaskGraph.tsx` | 已驗證 |
| `CR-207` | 2026-08-26 | [系統流程圖與關聯圖全螢幕、關聯線全段拖曳熱區與層級鎖定優化（Frame置底、Box置底、線段與卡片永不被遮蔽）](#cr-207) | `SystemFlow.tsx`, `TaskGraph.tsx`, `index.css`, `.gitignore` | 已驗證 |
| `CR-206` | 2026-08-26 | [刪除/永久刪除/還原提示彈窗詳載子項目種類統計與預覽清單（含任務單/問題單分類與徽章）](#cr-206) | `TaskDrawer.tsx`, `DeletedTasks.tsx` | 已驗證 |
| `CR-205` | 2026-08-26 | [已刪除事件支援清單階層樹狀呈現：父子縮排、收折展開箭頭、收納盒標示與原隸屬提示](#cr-205) | `DeletedTasks.tsx` | 已驗證 |
| `CR-204` | 2026-08-26 | [任務刪除與永久刪除全站獨立提示框化：取代原生 window.confirm 與標題列擠壓，提供清晰階層連帶說明彈窗](#cr-204) | `TaskDrawer.tsx`, `DeletedTasks.tsx` | 已驗證 |
| `CR-203` | 2026-08-26 | [收納盒與子卡片還原互動決策機制：階層關聯確認彈窗（一併還原/單獨還原/移至外層獨立卡片）](#cr-203) | `tasks.ts`, `api.ts`, `DeletedTasks.tsx`, `TaskGraph.tsx` | 已驗證 |
| `CR-201` | 2026-08-26 | [關聯圖接點顏色校正：還原左右紅色（排程相依）與上下紫色（階層關係）標準規則配色](#cr-201) | `TaskGraph.tsx` | 已驗證 |
| `CR-200` | 2026-08-26 | [已刪除事件資訊齊全化：種類徽章、在地化中文狀態標籤、原本進度條、問題指示與側欄說明更新](#cr-200) | `DeletedTasks.tsx`, `strings/nav.ts` | 已驗證 |
...
| `CR-189` | 2026-08-21 | [關聯圖連線粗細、自訂變色與接點升級：全線加粗為 2.5px、支援 8 種色彩選擇（含收納盒內）、接點放大為 20px 並四向多彩區分](#cr-189) | `TaskGraph.tsx` | 已驗證 |
| `CR-188` | 2026-08-21 | [關聯圖連線互動優化：方案 A+B 全線 36px 寬幅點擊熱區、懸停全線加粗發光與折點動態放大光暈](#cr-188) | `TaskGraph.tsx` | 已驗證 |
...
| `CR-183` | 2026-08-21 | [關聯圖排版修正：修復折疊後進度條被遮蔽問題，消除標頭橫向滑動與溢出拖拉](#cr-183) | `TaskGraph.tsx` | 已驗證 |
| `CR-182` | 2026-08-21 | [關聯圖折疊功能：收納盒與卡片支援內部子任務/問題單「▲ 摺疊 / ▼ 展開」與狀態持久化](#cr-182) | `TaskGraph.tsx`, `strings/flow.ts` | 已驗證 |
| `CR-181` | 2026-08-20 | [關聯圖刪線修復：移除全螢幕遮罩層、修正折點拖曳位移閾值與點擊刪除確認機制](#cr-181) | `TaskGraph.tsx` | 已驗證 |
| `CR-180` | 2026-08-20 | [關聯圖隔離規則：清理跨收納盒/跨內外舊連線、後端 API 擋關與父層異動自動清理連線機制](#cr-180) | `routes/links.ts`, `routes/tasks.ts`, `seed.ts` | 已驗證 |
...
| `CR-172` | 2026-08-20 | [行動裝置自適應：針對複雜頁面新增「建議使用電腦版觀看」友善提示與手機側欄自動收合](#cr-172) | `DesktopRecommendedNotice.tsx`, `App.tsx`, `EpicSidebar.tsx` | 已驗證 |
...
- **使用者需求**：行事曆 的顯示清單 現在會因為滑鼠移走而消失 改成稍微停留 讓滑鼠可以移上去 才能移動上面的卷軸。
- **實作與設計**：
  1. **延遲關閉緩衝 (`Calendar.tsx`)**：在 `DayCell` 加入 300ms 的延遲關閉緩衝計時器（`closeTimerRef`），當滑鼠從格子移出時不立刻銷毀彈窗，給予使用者平滑移入懸浮窗的操作時間。
  2. **移入彈窗保持與滾動啟用 (`Calendar.tsx`)**：在 Portal 懸浮視窗上掛載 `onMouseEnter`（立即清除關閉計時器）與 `onMouseLeave`（重新啟動 300ms 緩衝關閉），並將樣式由 `pointer-events-none` 改為 `pointer-events-auto`，完整支援滑鼠滾動內部多筆事件的上下捲軸。
...
- **使用者需求**：示範帳號已經幫你填好了，直接按登入就能看到含甘特、看板與對外詢問的示範資料。 這句話可以移除了。
- **排查與實作**：
  1. **移除登入頁示範帳號提示文字 (`Login.tsx`)**：
     - 自登入畫面中移除 `demoHint`（「示範帳號已經幫你填好了...」）說明區塊，使登入介面呈現更簡潔乾淨。
...
- **使用者需求**：幫我 google 登入暫時說明是無效的。
- **排查與實作**：
  1. **登入頁面 Google 登入禁用與警語提示 (`Login.tsx`)**：
     - 在登入頁將 Google 第三方登入按鈕固定呈現為停用狀態（`disabled`）並加上「暫時無效」醒目標籤。
     - 按鈕下方增加專屬提示文字「⚠️ Google 登入功能目前暫時無效，請使用上方 Email 與密碼登入。」引導使用者以帳號密碼登入。
  2. **帳號綁定頁面 Google 綁定按鈕同步停用 (`AccountPanel.tsx`)**：
     - 於帳號設定的「登入方式」清單中，當存在 Google 供應商時將其綁定按鈕設為停用並標註「（暫時無效）」。

### <a id="cr-221"></a>CR-221 (2026-09-03) — 專案設定頁支援專案公開性（公開 / 私人）切換與即時快取同步

- **使用者需求**：專案一開始有公開跟不公開，但是後續沒辦法調整，增加調整功能。
- **排查與實作**：
  1. **專案設定頁公開性設定區塊 (`ProjectSettings.tsx`)**：
     - 在專案設定頁新增 `ProjectVisibilitySection` 區塊，當前具備專案管理權限（建立者、管理者或超級管理者）時可檢視專案目前公開狀態（🌐 公開專案 或 🔒 私人專案）。
     - 提供一鍵切換按鈕，呼叫 `Api.patchProject(projectId, { isPublic })` 更新專案公開性，並即時失效 `project`、`projects` 與 `joinableProjects` 快取，使工作區成員之專案搜尋結果即刻同步更新。

### <a id="cr-220"></a>CR-220 (2026-09-03) — 專案軟刪除（標記刪除）與超級管理者專案還原管理功能

- **使用者需求**：刪除專案功能，僅限超級管理者或建立者可刪除，且改為標記刪除（軟刪除），之後只有超級管理者可以恢復整個專案。
- **排查與實作**：
  1. **後端專案軟刪除與權限控管 (`projects.ts`, `auth.ts`)**：
     - 新增 `DELETE /projects/:id` 端點：嚴格驗證僅**專案建立者（擁有者）**或**全域超級管理者（`isSuperAdmin`）**可執行刪除。執行時寫入 `UPDATE project SET archived_at = now()`，封存保留所有任務、關聯線與排版資料。
     - 在 `requireProjectRole` 檢查中加入 `AND p.archived_at IS NULL`，防止已刪除專案被一般 API 存取。
     - 新增 `GET /projects/deleted` 端點：僅限**全域超級管理者**可查詢所有已標記刪除之專案清單與相關統計。
     - 新增 `POST /projects/:id/restore` 端點：僅限**全域超級管理者**可將已刪除專案解除封存（`archived_at = NULL`）恢復使用。
  2. **前端 API 與專案設定頁危險區域 (`api.ts`, `ProjectSettings.tsx`)**：
     - 新增 `DeletedProject` 型別與 `deleteProject`、`deletedProjects`、`restoreProject` 前端 API 方法。
     - 首頁不放刪除按鈕以維持清爽並防止誤觸；在專案設定頁底部為建立者與超級管理者新增「⚠️ 危險區域：刪除專案」區塊與確認彈窗，刪除後自動返回專案首頁。
  3. **全站管理員還原面板 (`AdminPanel.tsx`)**：
     - 在系統管理面板（`AdminPanel`）為超級管理者新增「🗑️ 已刪除專案（還原管理）」分頁，完整列出已刪除專案之代碼、名稱、建立者、任務數與刪除時間，並提供一鍵「↺ 恢復專案」功能。
...
- **使用者需求**：檢查清單功能中已被收納的任務單是否會掉到收納盒外，並進行修復。
- **排查與實作**：
  1. **清單視圖收折防漏機制修復 (`List.tsx`)**：
     - 在清單視圖的樹狀遞迴展開函式（`walk`）中，原先當收納盒處於收折狀態（`collapsedSet.has(t.id)`）時會提早 `return`，導致子任務 ID 未被記錄至已處理集合（`processed`）。
     - 函式底部的「防漏兜底迴圈」（`for (const t of tasks) if (!processed.has(t.id)) ...`）會誤將未被標記的盒內子任務當成頂層獨立任務，強制以 `depth: 0` 重新插入清單最底部。
     - 實作 `markDescendants` 遞迴函式，在收折時遞迴將所有子孫任務 ID 標記為已處理（`processed`），徹底解決收折時盒內任務單掉出收納盒外的問題。
  2. **已刪除事件階層樹狀清單同步修復 (`DeletedTasks.tsx`)**：
     - 在已刪除事件的樹狀視圖 `walk` 函式中同步加入 `markDescendants` 標記已拜訪（`visited`），確保收納盒收折時，底下已刪除的子卡片正確保持隱藏，不掉出盒外。

### <a id="cr-217"></a>CR-217 (2026-08-31) — 子任務為自己負責時父任務連鎖紅框與置頂繼承顯示

- **使用者需求**：子任務紅框的話 父任務也要跟著紅框。
- **排查與實作**：
  1. **子孫任務指派連鎖紅框繼承 (`EpicSidebar.tsx`)**：
     - 在 `EpicSidebar` 以 DFS 遞迴巡訪所有任務子樹，計算 `hasMineSet` 集合；只要任何子任務、孫任務的指派人是目前登入者，其所有的上層父任務、大項目或收納盒皆自動納入紅框高亮目標。
     - 在 `TreeNode` 樹節點接收並向下遞迴傳遞 `hasMineSet`，當父任務包含自己負責的子任務時，同樣套用 `border-2 border-red-500` 醒目紅框，並在折疊狀態下依然能透過紅框與「含我的」標籤清晰辨識內部含有個人待辦項目。
  2. **父層選單項目自動連鎖置頂 (`EpicSidebar.tsx`)**：
     - 頂層 `epics` 與各層 `kids` 篩選改依 `hasMineSet` 進行判定，包含個人任務的父任務與收納盒同步置頂於選單最上方，確保個人工作區間快速可見。

### <a id="cr-216"></a>CR-216 (2026-08-31) — 任務負責人為自己之選單項目置頂排序與紅框醒目顯示

- **使用者需求**：任務負責人是自己的話 menu排序在最上面 並幫我加紅框顯示。
- **排查與實作**：
  1. **Menu 側欄負責人任務置頂排序 (`EpicSidebar.tsx`)**：
     - 引入 `useAuth()` 取得當前登入者 ID (`user.id`)，判定任務是否指派給自己（`t.assigneeId === user.id`）。
     - 在頂層選單大項目 (`epics`) 與各階層子任務清單 (`kids`) 中，優先將自己負責之任務抽出並排序在最頂端（`[...myEpics, ...otherEpics]` / `[...myKids, ...otherKids]`）。
     - 在自己負責的任務群組與其他任務之間自動繪製分隔線（`dividerAfterTaskIdSet`），版面條理分明。
  2. **紅框高亮與「我的」標籤強化 (`EpicSidebar.tsx`)**：
     - 在 `TreeNode` 任務列加上 `border-2 border-red-500 ring-1 ring-red-500/60` 醒目紅框與陰影。
     - 在任務編號與種類標籤旁增加專屬「我的」紅色標章，便於在複雜專案架構中瞬間鎖定個人負責任務。

### <a id="cr-215"></a>CR-215 (2026-08-31) — 超級管理者 vs 專案創立者（擁有者）權限範圍嚴格收斂：僅超級管理者可跨專案查看所有成員權限，專案擁有者僅能訪問有權限之專案

- **使用者需求**：只有超級管理者 可以查看所有專案的所有成員的權限 擁有者應該是專案創立者只能看有權限的專案。
- **權限模型精確重構實作**：
  1. **移除工作區管理者自動訪問專案旁路 (`auth.ts`)**：
     - 在 `requireProjectRole` 與 `requireProjectManager` 中移除 `ws_role IN ('OWNER', 'ADMIN')` 自動賦予專案 `MANAGER` 的機制。
     - 嚴格限定僅全域超級管理者（`isSuperAdmin`）、該專案的建立者（`created_by = user.id`，即專案擁有者）或專案成員中被指派為 `MANAGER` 者具備該專案管理權限。
  2. **專案清單可見性嚴格收斂 (`projects.ts`)**：
     - 在 `GET /projects` 中移除工作區擁有者與管理者的全專案查詢放行條件。
     - 僅全域超級管理者可跨專案查詢全站專案；專案建立者與一般成員僅能看到自己建立或已獲准加入的專案。
  3. **前後端身分屬性與權限判定對齊 (`auth.ts`, `api.ts`, `App.tsx`)**：
     - 在 `/auth/me`、`/auth/login`、`/auth/refresh` 回傳 `isSuperAdmin` 狀態。
     - 前端 `canManageProject` 與成員管理選單精確對齊，確保只有超級管理者、專案建立者（擁有者）或專案管理者可存取專案參數與成員審核設定。

### <a id="cr-214"></a>CR-214 (2026-08-31) — 專案資料範圍嚴格隔離檢查與修復：行事曆請假與表單專案成員範圍限縮，杜絕跨專案資料外洩

- **使用者需求**：每個功能都應該 不能跨專案顯示 只能在該專案出現 除了超級管理者 的權限控管能跨專案 你檢查一下。
- **全系統功能檢視與修復**：
  1. **全域視圖與 API 檢查確認**：
     - 清單（List）、看板（Board）、甘特圖（Gantt）、任務關聯圖（TaskGraph）、系統流程圖（SystemFlow）、遊樂場（Playground）、儀表板（Dashboard）、發文追蹤看板（InquiryBoard）、成員視圖（MembersView）、已刪除事件（DeletedTasks）、專案成員管理（MembersPanel）與專案參數設定（ProjectSettings）皆嚴格綁定 `projectId`，不存在跨專案資料洩漏。
     - 系統管理（AdminPanel）作為超級管理者 / 工作區管理者之權限控管功能，維持工作區層級管理。
  2. **行事曆請假紀錄專案隔離修復 (`Calendar.tsx`)**：
     - 排查發現原請假紀錄（`leaves`）取回工作區所有假單後未過濾專案成員，導致非當前專案成員之請假亦顯示於專案行事曆。
     - 加入 `projectMemberIds` 過濾，嚴格限定僅顯示當前專案成員及任務指派者之假單。
     - `LeaveDialog` 注入 `projectId`，表單挑選代登記對象與代理人全面限縮為當前專案成員名單。
...
- **使用者需求**：
  1. 不要 F5 回到首頁，應該是留在該頁做整個刷新。
  2. 版本上的建置時區顯示不對。
- **排查與修復實作**：
  1. **網址參數雙向同步與解析 (`App.tsx`)**：
     - 在 `App.tsx` 實作 `parseUrlState` 與 `updateUrlState`，將使用者的導覽狀態（`projectId`、`view`、`openTask`、`epicId`、`account`）無感即時同步至網址 Search Params（`?project=...&view=...&task=...&epic=...&account=...`）。
  2. **初次身分驗證生命週期保護與 F5 原頁全域刷新 (`App.tsx`)**：
     - 修復初次身分載入（`auth/refresh`）時 `seenUserId` 從 `null` 轉為真實 ID 被誤判為「換人登入」導致清空 `projectId` 與網址的 Bug。改用 `activeUserId` 嚴格在 `ready` 完成後且非首次載入才重設，F5 重新整理時精準保留專案、視圖頁籤、開啟任務與側欄大項目篩選，並重新拉取後端最新資料。
  3. **版本號與建置時區校正 (`vite.config.ts`, `version.ts`)**：
     - 使用 `Intl.DateTimeFormat` 指定 `Asia/Taipei`（UTC+8）格式化建置時間，徹底解決 Docker Alpine 容器內預設 UTC 時區造成建置時間偏差 8 小時問題；同步更新版本標籤為 `v0.1.0-CR211`。
  4. **上一頁/下一頁歷史導覽與首頁狀態相容 (`App.tsx`)**：
     - 監聽 `popstate` 事件以無縫支援瀏覽器上一頁/下一頁前進後退；「切換專案」或登出時精準清空網址 Query String 並回退至專案選擇頁；專案清單載入後自動校驗專案有效性。

### <a id="cr-210"></a>CR-210 (2026-08-27) — 正交折線接點緩衝間距擴展與折角貼近優化（提升至 40px、直連同軸直通、修復化簡壓縮）

- **使用者需求**：不要讓 90 度的線跟接點太接近，都擠在一起了。
- **排查與修復實作**：
  1. **擴展正交折線緩衝間距 (`orthogonalRouting.ts`)**：
     - 將預設緩衝間距（`margin`）由 20px 擴展至 40px，確保連線從起點接點出發或進入終點接點時，至少具備 40px 的筆直延伸安全淨空。
  2. **重構直角轉折幾何計算與同軸直通 (`orthogonalRouting.ts`)**：
     - 重構 `buildOrthogonalPath` 水平/垂直及轉折線型邏輯；當兩端接點同軸同向且自然通達時，自動採用 100% 筆直直線連接，不產生多餘轉折。
     - 修復 `simplifyPoints` 共線化簡誤將出發緩衝段壓縮的問題，杜絕 90 度折點緊貼接點把手（Handle）造成的擁擠與視覺重疊。

### <a id="cr-209"></a>CR-209 (2026-08-26) — 流程圖關聯線容器穿透與步驟避障（排除 Box 障礙物允許跨容器自由穿行，嚴格保留 Step 步驟避障）

- **使用者需求**：流程圖關聯線可以穿透容器，但是要避開步驟。
- **排查與修復實作**：
  1. **排除容器框避障障礙 (`orthogonalRouting.ts`)**：
     - 在 `getObstaclesFromNodes` 障礙物提取演算法中，將所有模組收納盒／泳道框（Box / Container）自障礙物陣列中排除（`if (isBox) continue`）。
     - 允許流程關聯線自然、平滑地穿行跨越任何容器框與泳道邊界，不再強制繞行整個大容器外圍。
  2. **嚴格保留步驟卡片避障 (`orthogonalRouting.ts`)**：
     - 各步驟實體卡片（Step / Task）依然嚴格計算為幾何障礙物（`isSource` 與 `isTarget` 邊界防護），直角正交折線演算法會自動計算多通道最短無碰撞路徑，絕不切穿任何步驟卡片本體。

### <a id="cr-208"></a>CR-208 (2026-08-26) — 系統流程圖連線自訂變色支援與關聯圖/流程圖過近折點把手自動隱藏（消除相鄰卡片擁擠）

- **使用者需求**：
  1. 流程圖的線條要能更換成不同顏色。
  2. 關聯線接點跟接點太近的話，不要出現移動用的折點把手（圓點），避免畫面過於擁擠。
- **排查與修復實作**：
  1. **系統流程圖連線自訂變色 (`SystemFlow.tsx`)**：
     - 在點擊連線彈出的設定 Modal 中新增 9 色調色盤（靛青藍、經典藍、翠玉綠、優雅紫、活力橘、熱情紅、深沉灰、晴空青、玫瑰粉）。
     - 點選儲存後同步更新 `style.stroke`、`markerEnd.color` 與 `data.color`，並落盤持久化至畫布文件。
     - `styledEdges`、`loadInitialPages`、`handleSwitchPage` 與 `handleDeletePage` 全面相容讀取與呈現自訂連線顏色。
  2. **過近接點折點把手自動隱藏 (`SystemFlow.tsx`, `TaskGraph.tsx`)**：
     - 在 `FlowLabeledEdge` 與 `OrthogonalEdge` 中引入 `isTooClose` 判斷：當兩端接點距離 `< 70px` 且使用者未手動拉出自訂折點時，自動隱藏中央拖曳圓點把手。
     - 徹底解決相鄰/緊鄰卡片間連線短於 70px 時，中央折點與兩端接點重疊擠在一起的視覺擁擠問題；全線點擊開窗設定與寬幅感應功能依然完好保留。

### <a id="cr-207"></a>CR-207 (2026-08-26) — 系統流程圖與關聯圖全螢幕、關聯線全段拖曳熱區與層級鎖定優化（Frame置底、Box置底、線段與卡片永不被遮蔽）

- **使用者需求**：
  1. 流程圖與關聯圖支援全螢幕功能，且圖示需與「顯示全部（FitAll）」明確區分。
  2. 流程圖關聯線可隨意拖曳調整路徑。
  3. 移動框體時會蓋住連線與卡片，需修復層級遮蔽問題。
  4. 遠距投保系統流程圖改為「左側前端區域 ↔ 右側後端區域」架構。
  5. 避免將大檔 docx/pdf 誤提交至版控庫。
- **排查與修復實作**：
  1. **全螢幕模式與圖示區隔 (`SystemFlow.tsx`, `TaskGraph.tsx`)**：
     - 在頂部獨立工具列右側與左下角 ReactFlow Controls 同步新增全螢幕切換按鈕。
     - 採用標準對角擴展雙箭頭圖示（`⤢` 全螢幕 / `⤡` 縮小），完整支援鍵盤 Esc 同步退出。
     - 與「顯示全部（FitAll）」框選視角圖示（`[ ▢ ]`）形成清晰視覺區隔。
  2. **全線段拖曳熱區與直覺折點控制 (`SystemFlow.tsx`)**：
     - 在 `FlowLabeledEdge` 擴充 24px 寬幅透明感應軌道，滑鼠懸浮或按住連線任意處即可直接拖曳改變路徑與 Waypoint 座標。
     - 加大同色高對比發光轉角把手（`h-3.5 w-3.5`、`after:-inset-[14px]`），支援雙擊還原自動避障直角路徑。
  3. **畫布層級嚴格鎖定 (z-index) (`index.css`, `SystemFlow.tsx`)**：
     - Frame 標示框鎖定最底層（`z-index: -2`）。
     - Box 模組/泳道收納盒鎖定底層（`z-index: 0`），拖曳或選取中永不提升遮蔽卡片與連線。
     - Step/Task 流程卡片置於 `z-index: 5`（拖曳時 `20`）。
     - Edges 關聯線置於 `z-index: 15`，Edge Labels 與把手置於 `z-index: 25`~`50`。
     - `orderParentNodesFirst` 與 `onNodesChange` 拖曳結束時自動維護 Frame ➔ Box ➔ Step 節點陣列順序。
  4. **遠距投保端對端全流程 5 層與左/右分區拓撲重構**：
     - 重構 Page 1 為左側【前端應用與使用者終端區域 (ePOS, 業務端, 客戶端)】與右側【後端服務與第三方介接區域 (meet-service, API Gateway, SOAP SMS, TWCA, eKYC, S3)】。
     - 完整發布至遠端 Live 實例畫布（54 節點、58 條連線）。
  5. **版本庫過濾防護 (`.gitignore`)**：
     - 納入 `*.docx`、`*.doc`、`*.pdf`、`ch*_body.txt`、`docx_*.txt` 等過濾規則。

### <a id="cr-206"></a>CR-206 (2026-08-26) — 刪除/永久刪除/還原提示彈窗詳載子項目種類統計與預覽清單（含任務單/問題單分類與徽章）

- **使用者需求**：刪除提示窗確認是否有寫內含幾個問題單、任務單之類。
- **排查與修復實作**：
  1. **任務詳情抽屜刪除提示窗 (`TaskDrawer.tsx`)**：
     - 新增 `childTypeSummary` 統計，若該收納盒內含子卡片，明確提示「底下包含 N 個子項目（X 個任務單、Y 個問題單）」，並在提示窗內以微縮捲軸列表逐一列出包含的子卡片編號、種類徽章與標題。
  2. **已刪除事件還原與永久刪除提示窗 (`DeletedTasks.tsx`)**：
     - 在還原收納盒提示窗中，詳細註明「底下包含 N 個已刪除子項目（X 個任務單、Y 個問題單）」並呈現子項目清單。
     - 在永久刪除收納盒提示窗中，警示並條列連帶徹底清除的子卡片明細與種類分類。

### <a id="cr-205"></a>CR-205 (2026-08-26) — 已刪除事件支援清單階層樹狀呈現（父子縮排、收折展開箭頭、收納盒標示與原隸屬提示）

- **使用者需求**：已刪除事件無法知道關聯關係，改用跟清單一樣的階層。
- **排查與修復實作 (`DeletedTasks.tsx`)**：
  1. **階層樹狀排序演算法**：
     - 在已刪除事件清單中以 `parentId` 建立父子映射樹，根節點（收納盒優先）以深度優先（DFS）演算法展開計算 `depth` 與 `hasKids`。
     - 支援各層級 `collapsedSet` 記憶，點擊 `▸` / `▾` 箭頭即可折疊/展開子卡片分支。
  2. **視覺階層元素比照清單視圖**：
     - 子卡片以 `depth * 22px` 動態縮排，並繪製 `└` 階層引導分支線。
     - 收納盒前置 `📦` 圖示，標題加粗醒目呈現。
     - 若子卡片單獨被刪除（父收納盒仍在專案中或未一起展開），自動於標題旁標註 `↳ 原隸屬收納盒：【MRG-X】名稱` 標籤，關聯來龍去脈一目了然。

### <a id="cr-204"></a>CR-204 (2026-08-26) — 任務刪除與永久刪除全站提示框化（自訂 Modal 取代原生 confirm 與標題列擠壓）

- **使用者需求**：刪除任務改用提示框顯示。
- **排查與修復實作**：
  1. **任務詳情抽屜刪除提示框 (`TaskDrawer.tsx`)**：
     - 移除抽屜頂部標題列原先的行內文字兩段式確認（避免擠壓標題與按鈕空間）。
     - 點擊「刪除」按鈕時開啟專用提示彈窗（Modal），清晰提示任務編號與標題；若包含子任務，清楚告知「⚠️ 此任務為收納盒，底下包含 N 個子任務，將一併連帶移至已刪除事件」。
  2. **已刪除事件永久刪除提示框 (`DeletedTasks.tsx`)**：
     - 移除瀏覽器原生 `window.confirm`，改為使用獨立的紅色警示 Modal 提示框，確保視覺體驗一致性與防誤觸確認。

### <a id="cr-203"></a>CR-203 (2026-08-26) — 收納盒與子卡片還原互動決策機制（一併還原/單獨還原/移至外層獨立卡片）

- **使用者需求**：
  1. 還原如果是子卡片，若收納框已被刪除，要詢問要不要連同收納框一起還原；若是單獨還原子卡片，則移到外層畫布作為獨立卡片，解除 parentId 連線限制。
  2. 若還原收納盒自己且底下包含已刪除子卡片，要詢問要一併還原還是單獨還原（保留為空盒）。
  3. 子卡片還原後若本來在收納盒內且收納盒正常存在，要直接放回該收納盒內。
- **實作細節**：
  1. **後端還原模式擴充 (`tasks.ts`, `api.ts`)**：
     - `POST /tasks/:id/restore` 支援 `mode: 'all' | 'self_only' | 'detach_parent'`。
     - `all`：透過 `task_closure` 連帶還原自身與所有子孫任務。
     - `self_only`：僅將自身的 `deleted_at` 設為 `NULL`，子卡片仍保留已刪除狀態。
     - `detach_parent`：將自身 `deleted_at` 設為 `NULL` 且 `parent_id = NULL`，重建閉包成為最外層獨立卡片。
  2. **前端階層還原確認彈窗 (`DeletedTasks.tsx`)**：
     - **收納盒底下有刪除子卡片時**：跳出提示「包含 N 張已刪除子卡片」，提供「📦 一併還原收納盒與全部子卡片」與「🗃️ 僅還原收納盒自己」。
     - **子卡片的收納盒也已刪除時**：跳出提示，提供「📦 連同收納盒一起還原（卡片放回盒內）」與「📄 僅還原此子卡片（移至最外層獨立卡片）」。
     - **子卡片的收納盒未被刪除時**：提示確認還原並自動放回該收納盒內。
  3. **關聯圖收納模式自動保證 (`TaskGraph.tsx`)**：
     - 凡任務底下含有子卡片（`kids.length > 0`），強制以 `box` 模式呈現，杜絕塌陷為單卡導致子卡片丟失或無法連線。

### <a id="cr-201"></a>CR-201 (2026-08-26) — 關聯圖接點顏色校正（還原左右紅色、上下紫色標準配色）

- **使用者需求**：關聯圖的接點顏色跟規則不一致。
- **排查與修復實作 (`TaskGraph.tsx`)**：
  1. **標準配色校正**：排查發現接點（Handles）先前被誤設為四向四色（左藍/右紅/上綠/下紫）。依 PMFlow 排程與階層規範重新校正：
     - **左右接點（排程相依）**：`left-in`、`left-out`、`right-in`、`right-out` 統一設為 **紅色**（`#ef4444`）。
     - **上下接點（階層關係）**：`top-in`、`top-out`、`bottom-in`、`bottom-out` 統一設為 **紫色**（`#8b5cf6`）。
  2. 與連線出發樣式（左右紅色實線、上下紫色虛線）達成 100% 視覺一致性。

### <a id="cr-200"></a>CR-200 (2026-08-26) — 已刪除事件資訊齊全化：種類徽章、在地化中文狀態標籤、原本進度條、問題指示與側欄說明更新

- **使用者需求**：
  1. 已刪除事件看不出是什麼單（任務單/問題單）、原本進度是多少、todo 是什麼狀態。
  2. 現在沒有大項目了，側欄「還沒有大項目。大項目就是把一件大事分成幾塊...」這段描述需要更新。
- **排查與修復實作 (`DeletedTasks.tsx`, `strings/nav.ts`)**：
  1. **種類徽章與問題單指示**：在 `DeletedTasks.tsx` 查詢專案參數建立 `typesMap`，表格新增「種類」欄位呈現 `<TypeBadge />`，標題旁若有遭遇問題則顯示 `<ProblemBadge />`。
  2. **在地化中文狀態標籤**：依專案自訂狀態對照轉換 `todo` / `in_progress` / `done` 為「待處理」、「進行中」、「已完成」等分類徽章色彩樣式。
  3. **原本進度條展示**：新增「原本進度」欄位，顯示可視化進度長條與百分比數據。
  4. **移除過時「大項目」說明**：更新 `strings/nav.ts` 中的 `sidebar.emptyTitle` 與 `sidebar.emptyHint` 為「目前專案尚無任務。點擊右上角「＋」建立第一張任務，可於任務內掛載子任務或切換為收納盒進行分工組織。」。
...
- **使用者需求**：
  1. 關聯圖與流程圖各自獨立；不同頁面共用編輯要分開。
  2. 增加超級管理員/專案擁有者可設定那些人可以改流程圖跟關聯圖（授權功能）。
  3. 授權按鈕不要跟著右側工具列數量跑動，統一放在固定位置。
  4. 流程圖裡面縮放標示框壞掉無效修復；區域標示框移動與選取時必須永遠在最底層。
  5. 流程圖換分頁時不得殘留上一頁的選取與快取資訊。
  6. 流程圖分頁標籤移除微調按鈕與複製按鈕，支援雙擊更名，刪除按鈕常駐顯示且開放刪除所有分頁，最後一頁刪除後自動產生全新空白分頁。
- **排查與修復實作 (`SystemFlow.tsx`, `TaskGraph.tsx`, `canvas.ts`, `skills.ts`, `0028_canvas_permissions.sql`, `index.css`)**：
  1. **畫布編輯權限白名單體系**：
     - 新增 `0028_canvas_permissions.sql` 建立 `canvas_permission` 資料表，支援 `task-graph` 與 `system-flow` 獨立設定編輯白名單。
     - 實作 `GET/PUT /api/v1/projects/:projectId/canvas-permissions/:canvasKey`，專案建立者、Owner、Manager 或系統管理員可配置成員名單。
     - 前端畫布頂部工具列左側標題旁固定放置「⚙️ 授權」按鈕與「🔒 唯讀」標籤。
  2. **區域標示框（Frame）移動與靜止永遠置底**：
     - `orderParentNodesFirst` 排序將 Frame 節點置於陣列最底層（`zIndex: -1`）。
     - 全域樣式加入 `.react-flow__node-frame.dragging, .react-flow__node[data-type="frame"].dragging { z-index: -1 !important; }`，拖曳移動時絕不遮蔽卡片、模組盒與連線。
  3. **流程圖分頁切換資料隔離與防抖落盤**：
     - 新增 `flushCurrentPageChanges`，切換分頁時立即結算前頁未存變更，重置選取、拖曳暫存、拉線起點與彈窗狀態。
     - 依分頁個別儲存與還原 Viewport 焦點視角。
  4. **流程圖分頁操作簡化與常駐刪除**：
     - 移除微調按鈕（`◀`/`▶`）與複製按鈕（`📑`），支援雙擊直接編輯標籤名稱。
     - 刪除按鈕（`×`）常駐顯示，授權編輯者可刪除所有分頁（包含第 1 頁）；刪除最後一個分頁時自動產生全新空白「流程圖 1」分頁。
  5. **AI 技能規格端點擴充**：
     - 在 `/skills` 探索端點中加入 `get_canvas_doc`、`save_canvas_doc`、`get_canvas_permissions` 與 `update_canvas_permissions` 規格。

### <a id="cr-191"></a>CR-191 (2026-08-21) — 關聯圖收納盒連線顏色還原、純 CSS 零跳動懸停發光、折點中心鎖定與 E2E 測試全數對齊修復

- **使用者需求**：關聯線放上後，直角的點會亂跑；先把收納盒裡的關聯線顏色還原，保留滑鼠放上去變色功能。全部都要測試並修正，不然推上 github 會錯誤。
- **排查與修復實作 (`TaskGraph.tsx`, `orthogonalRouting.ts`, `routes/auth.ts`, `e2e.sh`, `e2e.test.ts`)**：
  1. **收納盒連線預設顏色還原**：
     - `getEdgeStyleAndMarker`、`onEdgeClick` 與 `styledEdges` 全面還原為經典標準配色（左右出發紅色 `#ef4444`、上下出發紫色 `#8b5cf6`），保留彈窗 8 色自訂變色支援。
  2. **純 CSS 零跳動懸停發光與穿透修正**：
     - 移除 Edge 元件內的 `useState(isHovered)` 狀態監聽，全面改用 Tailwind `peer/hit` 與 CSS `:hover` 硬體加速，徹底消除滑鼠移動時的組件重繪震盪。
     - 收納盒本體背景改為 `pointer-events-none`（標頭與控制鈕維持 `pointer-events-auto`），滑鼠懸停於收納盒內部連線上可 100% 精準觸發全線加粗與亮藍發光高亮。
  3. **直角折點中心精確鎖定與計算穩定化**：
     - 折點 DOM 容器統一採用 `translate(-50%, -50%) translate(${px}px, ${py}px)`，確保縮放時原點絕對鎖定於中心。
     - 在 `getPolylineMidpoint` 中引入確定性幾何中點演算法：3 節點鎖定唯一轉折角、4 節點鎖定中央垂直/水平主段中心，徹底杜絕折點在兩端 Stubs 之間跳動。
  4. **端對端整合測試 (E2E) 全面對齊與通過 (165/165 通過)**：
     - 修復 `auth.ts` 之 `/auth/impersonate` 支援超級管理者身分切換授權。
     - 校正 `e2e.sh` 第 4 項專案數判定、第 14 項同收納盒環偵測、第 15 項父子依賴約束與第 26 項 CR-180 跨收納盒連線隔離驗證。
     - 擴充 `e2e.test.ts` 跨平台測試套件，補齊 CR-165 AI 技能規格探索端點與 CR-180 關聯隔離防護驗證。
- **異動模組**：`apps/web/src/pages/TaskGraph.tsx`, `apps/web/src/lib/orthogonalRouting.ts`, `apps/api/src/routes/auth.ts`, `apps/api/test/e2e.sh`, `apps/api/test/e2e.test.ts`。

### <a id="cr-190"></a>CR-190 (2026-08-21) — 關聯圖收納盒內連線專屬辨識色：預設自動賦予翡翠綠/青綠區隔色彩，並完整支援自訂選色

- **使用者需求**：收納盒內的關聯線好像沒有變色。
- **排查與修復實作 (`TaskGraph.tsx`)**：
  1. **收納盒內連線專屬預設識別色**：
     - 在 `styledEdges` 與 `getEdgeStyleAndMarker` 中引入 `isInsideBox` 拓撲判定（比對 `sourceNode.parentId && sourceNode.parentId === targetNode.parentId`）。
     - **收納盒內部子卡片連線**：預設自動賦予鮮明的翡翠綠 `#10b981`（左右接點）與青綠色 `#06b6d4`（上下接點），與外部大連線（紅/紫）形成鮮明對比，無須手動設定即可一眼區分。
     - **自訂選色優先覆蓋**：若使用者透過連線彈窗自訂色彩，則優先尊重使用者選擇的自訂色彩。
  2. **彈窗初始選色校正**：
     - 在 `onEdgeClick` 中同步計算 `isInsideBox` 並將預設翡翠綠/青綠傳遞至 `confirmDeleteEdge` 狀態，使色塊選取器能正確預先高亮盒內預設色。
- **異動模組**：`apps/web/src/pages/TaskGraph.tsx`。

### <a id="cr-189"></a>CR-189 (2026-08-21) — 關聯圖連線粗細、自訂變色與接點升級：全線加粗為 2.5px、支援 8 種色彩選擇（含收納盒內）、接點放大為 20px 並四向多彩區分

- **使用者需求**：關聯線加粗，收納盒內的線也要能變色，連結點也要變大變色。
- **排查與修復實作 (`TaskGraph.tsx`)**：
  1. **關聯線基礎粗細全面升級**：
     - `getEdgeStyleAndMarker` 與 `styledEdges` 基礎線寬由 2px 提升至 2.5px，懸停加粗至 4px，箭頭尺寸由 14px 增至 16px，視覺層次更加清晰穩健。
  2. **連線自訂變色機制（含收納盒內連線）**：
     - 引入 `EDGE_COLOR_OPTIONS` 提供 8 種色彩選擇（預設紅 `#ef4444`、科技藍 `#3b82f6`、翡翠綠 `#10b981`、紫羅蘭 `#8b5cf6`、琥珀橘 `#f59e0b`、玫瑰粉 `#ec4899`、青綠色 `#06b6d4`、深岩灰 `#475569`）。
     - 連線彈窗新增「連線顏色」色塊選擇器，無論是收納盒內部子卡片連線或外部連線，均可自由變更顏色並同步保存至 `edgeColors` 本地與後端 `task-graph-extra`。
  3. **接點 (Handles) 與折點全面加大且四向變色**：
     - 卡片與收納盒的 4 向 Handles 接點由 16px（`w-4 h-4`）放大至 20px（`!w-5 !h-5`），並賦予四向獨立識別色彩：左藍（`#3b82f6`）、右紅（`#ef4444`）、上綠（`#10b981`）、下紫（`#8b5cf6`），支援 `hover:scale-125` 動態回饋。
     - 折點（Waypoint Dot）基礎尺寸由 10px 增至 14px（`h-3.5 w-3.5`），懸停時放大至 20px（`h-5 w-5`），背景色自動同步關聯線之即時色彩。
- **異動模組**：`apps/web/src/pages/TaskGraph.tsx`。

### <a id="cr-188"></a>CR-188 (2026-08-21) — 關聯圖連線互動優化：方案 A+B 全線 36px 寬幅點擊熱區、懸停全線加粗發光與折點動態放大光暈

- **使用者需求**：連結線的點不好點，要求實作推薦之「方案 A + B 組合（全線任意處可點 ＋ 懸停動態放大與發光光暈）」。
- **排查與修復實作 (`TaskGraph.tsx`)**：
  1. **方案 B 全線 36px 寬幅點擊與懸停熱區**：
     - 在 `OrthogonalEdge` 中增加 36px 寬幅透明感應層（`pointerEvents="stroke"`），滑鼠靠近整條連線的任意位置即可輕鬆觸發懸停高亮。
     - 點擊整條連線任意處（不侷限於折點）皆可直接彈出刪除關聯確認窗（`setConfirmDeleteEdge`）。
     - 連線懸停或選中時，自動由 2px 加粗至 3.5px，線條轉為亮藍色並附帶柔和藍色外發光（`drop-shadow(0 0 4px rgba(59, 130, 246, 0.6))`）。
  2. **方案 A 折點動態放大與發光光暈**：
     - 平時維持 10px 精緻小點（`h-2.5 w-2.5`），不干擾畫面整體排版。
     - 懸停或選中時，折點自動平滑放大至 16px（`h-4 w-4`，放大 1.25x），轉為鮮明藍色背景、白色邊框與 4px 半透明光暈外圈（`ring-4 ring-blue-400/35 shadow-md shadow-blue-500/50`）。
     - 折點拖曳與點擊熱區擴大至 44px（`after:-inset-[14px]`），隨手點擊或拖曳皆能精準命中。
- **異動模組**：`apps/web/src/pages/TaskGraph.tsx`。
...
- **使用者需求**：收納盒轉回卡片好像有其他關聯線閃爍，幫我檢查一下。
- **排查與修復實作 (`TaskGraph.tsx`)**：
  1. **排查原因**：
     - 在 `executeToggleMode` 中切換模式時，僅更新了 `toggledModes` 與子卡片的 `nodes`，未同步在該 frame 的 `setNodes` 中原子性更新目標節點自身之尺寸與模式。
     - `nodesWithHandlers` 之前未即時讀取 `toggledModes`，導致渲染時在「第 1 幀（舊尺寸與舊模式）」與「第 2 幀（`useEffect[tasks]` 新尺寸）」之間產生 16ms 狀態時差，導致周圍直角避障連線短暫重新計算跳動產生閃爍。
  2. **解決方案**：
     - 在 `executeToggleMode` 中原子同步更新 `setNodes` 目標節點之 `width: 256`, `mode: 'card'` 與 `style`。
     - 在 `nodesWithHandlers` 中即時以 `toggledModes[node.id]` 解析最新模式，徹底消除中間幀時差，達到零延遲無閃爍切換。
- **異動模組**：`apps/web/src/pages/TaskGraph.tsx`。

### <a id="cr-185"></a>CR-185 (2026-08-21) — 關聯圖收納盒還原機制：巢狀收納盒轉回卡片時，子卡片自動移入上層收納盒並自動網格排版

- **使用者需求**：解除關係後要把卡片變回平行關係，如果原本的收納盒有上游（上層收納盒），要回到上游的收納裡面，轉成上一層收納盒的移入動作自動排列。
- **排查與修復實作 (`TaskGraph.tsx`)**：
  1. **上層收納盒繼承與移入動作模擬**：
     - 當收納盒 `B`（位於上層收納盒 `P` 內）切換還原為一般卡片時，盒內的子卡片不再轉為畫布絕對座標，而是自動繼承 `parentId: P.id`。
     - 系統自動掃描 `P` 內現有佔用槽位，依序指派下一個空槽（`24 + col * 280, 110 + row * 115`），自動完成標準網格排列，與原卡片成為同層平行卡片。
  2. **最外層收納盒還原維持原地展開**：
     - 若 `B` 位於最外層畫布，子卡片轉為 `parentId: null` 並於原位置自然展開。
- **異動模組**：`apps/web/src/pages/TaskGraph.tsx`。

### <a id="cr-184"></a>CR-184 (2026-08-21) — 關聯圖排版優化：將「(移入卡片自動擴大容量)」提示字由頂部標頭移至收納盒底部邊框上方

- **使用者需求**：移入卡片自動擴大容量的字，移到每個卡片（收納盒）的底框的上面顯示。
- **排查與修復實作 (`TaskGraph.tsx`)**：
  1. **移除頂部標頭提示**：自收納盒頂部標頭第一行移除 `(移入卡片自動擴大容量)` 說明字，使標頭操作區更為清爽寬敞，專注於模式切換、Ref 編號與折疊按鈕。
  2. **底部邊框上方展示**：將該提示字移至收納盒內部最下方、底框正上方（`px-3 py-1 text-[10px] text-slate-400/80`），並在收納盒展開狀態下顯示，視覺更平衡。
- **異動模組**：`apps/web/src/pages/TaskGraph.tsx`。

### <a id="cr-183"></a>CR-183 (2026-08-21) — 關聯圖排版修正：修復折疊後進度條被遮蔽問題，消除標頭橫向滑動與溢出拖拉

- **使用者需求**：摺疊之後進度條消失了；收納跟展開功能導致出現了左右拖拉，不要出現拖拉。
- **排查與修復實作 (`TaskGraph.tsx`)**：
  1. **進度條自適應高度修復**：
     - 折疊時取消外層容器與節點物件的硬編碼固定高度（`height: undefined`, `min-h-[90px]`），讓收納盒與卡片能依據標題行數、警示標籤自然撐開，確保底部的 `<NodeProgressBar>` 100% 完整展示不被截斷。
  2. **消除標頭橫向滑動與溢出拖拉**：
     - 移除收納盒標頭的 `overflow-x-auto`，改為 `overflow-hidden`。
     - 修正卡片標頭的 `w-max whitespace-nowrap overflow-visible` 為 `flex justify-between w-full min-w-0 overflow-hidden`，徹底杜絕因標頭寬度超出 256px 產生的橫向滾動與左右拖拉。
- **異動模組**：`apps/web/src/pages/TaskGraph.tsx`。

### <a id="cr-182"></a>CR-182 (2026-08-21) — 關聯圖折疊功能：收納盒與卡片支援內部子任務/問題單「▲ 摺疊 / ▼ 展開」與狀態持久化

- **使用者需求**：收納盒跟卡片如果內部有卡片或問題單，多增加一個摺疊功能。
- **實作重點**：
  1. **條件式折疊按鈕 (`TaskGraph.tsx`, `strings/flow.ts`)**：
     - 在收納盒或卡片內部含有子任務（`childCount > 0`）或問題單（`problemCount > 0`）時，標頭種類徽章旁顯示精緻折疊按鈕（`▲ 摺疊` / `▼ 展開`）。
  2. **摺疊狀態響應與遞迴隱藏**：
     - **收納盒折疊**：內部所有子卡片與子連線自動隱藏（`hidden: true`），收納盒框體自適應縮減為單行緊湊卡片（高度 90px），標頭完整保留子任務數與各類警示徽章（問題單、逾期、詢問等）。
     - **卡片折疊**：自動遞迴隱藏其附屬之子任務與問題單卡片及相應連線。
     - **展開**：重新顯示所有子卡片/問題單與內部連線，收納盒自動計算網格尺寸還原寬高。
  3. **狀態持久化**：
     - 按專案 `projectId` 將折疊偏好儲存於 `localStorage`（`pmflow_simple_graph_collapsed_${projectId}`），切換檢視或重新整理頁面後維持折疊狀態。
- **異動模組**：`apps/web/src/pages/TaskGraph.tsx`, `apps/web/src/strings/flow.ts`。

### <a id="cr-181"></a>CR-181 (2026-08-20) — 關聯圖刪線修復：移除全螢幕遮罩層、修正折點拖曳位移閾值與點擊刪除確認機制

- **使用者需求**：為什麼又造成畫布的線無法刪除？
- **排查與修復實作**：
  1. **排查原因**：
     - 上一輪在 `<EdgeLabelRenderer>` 內嘗試注入全螢幕 `<svg>` 覆蓋層以捕獲點擊，但因 React Flow 的 `EdgeLabelRenderer` 容器處於螢幕未縮放平移空間，導致路徑座標錯位且多條連線的 `<svg>` 互相堆疊遮擋了畫布，進而攔截並破壞了 React Flow 原生 `<BaseEdge>` 的點擊事件。
     - 折點（Waypoint）的 `handlePointerDown` 無條件觸發防護鎖 `armWaypointClickGuard()`，導致使用者單擊折點時被誤判為拖曳放開，從而未觸發 `onEdgeClick`。
  2. **解決方案 (`TaskGraph.tsx`)**：
     - 徹底移除 `<EdgeLabelRenderer>` 內的全螢幕 `<svg>` 遮罩，恢復 React Flow 原生 `<BaseEdge interactionWidth={30}>` 乾淨透明的點擊響應。
     - 優化折點拖曳判定：僅在指針位移超過 3px（`hasMovedRef.current`）時才啟動拖曳與防護鎖；純單擊折點時不觸發位移防護，直接觸發 `onEdgeClick` 開啟刪除確認彈窗。
- **異動模組**：`apps/web/src/pages/TaskGraph.tsx`。

### <a id="cr-180"></a>CR-180 (2026-08-20) — 關聯圖隔離規則：清理跨收納盒/跨內外舊連線、後端 API 擋關與父層異動自動清理連線機制

- **使用者需求**：收納盒外的線不能連到收納盒內，如果目前有這樣的線先幫我刪除。
- **排查與修復實作**：
  1. **資料庫跨收納盒/跨內外無效連線即刻清理 (`task_link`)**：
     - 排查資料庫既有連線，發現示範專案中含有 2 條舊示範資料之跨收納盒連線（MRG-2 連至 MRG-6、MRG-7 連至 MRG-9），已直接執行 SQL 予以全數刪除。
  2. **種子資料校正 (`seed.ts`)**：
     - 修正 `seed.ts` 中示範任務連線為收納盒層級連線（MRG-1 前置準備連至 MRG-5 採購與施工、MRG-5 連至 MRG-8 遷移與切換），杜絕後續測試重置時再生成跨盒連線。
  3. **後端 API 建立關聯擋關 (`routes/links.ts`)**：
     - 於 `POST /tasks/:id/links` 增加強制約束：比對 `source` 與 `target` 之 `parent_id`，若兩者不一致（例如一內一外、或位於不同收納盒）則拋出 409 Conflict 阻擋建立。
  4. **父層變更自動清理違規連線 (`routes/tasks.ts`)**：
     - 於 `PATCH /tasks/:id` 與 `PATCH /tasks/:id/move` 在任務父層變更（`parentId` 異動）時，自動檢查並刪除因此變得不合法的跨邊界關聯線。
- **異動模組**：`apps/api/src/routes/links.ts`, `apps/api/src/routes/tasks.ts`, `apps/api/src/seed.ts`。
...
- **使用者需求**：行事曆裡的清單沒有跟著調（仍為多欄表格）；手機版只有清單內的任務可以進任務修改頁，其他檢視一律唯讀，並於唯讀模式增加美觀提示橫幅（方案 A）。
- **實作細節**：
  1. **行事曆週檢視手機單欄滿版卡片流 (`Week.tsx`)**：
     - 在手機端（`< 768px`），將週檢視及行事曆月視角展開週任務之 `52rem` 五欄表格重構為 100% 滿版單欄垂直卡片流（`MobileTaskCard`），完整包含類型色標、MRG 編號、標題、狀態徽章、負責人頭像、起訖日、進度條與各類警示徽章（問題單、逾期、對外詢問），徹底消除橫向溢出。
  2. **手機版專屬權限切分與唯讀提示橫幅 (`TaskDrawer.tsx`, `App.tsx`)**：
     - 手機版僅在「清單（List）」檢視下開放編輯與儲存；其餘檢視（看板、行事曆/週檢視、側欄、圖表等）開啟任務抽屜時一律強制帶入 `readOnly={true}` 唯讀模式。
     - 唯讀模式下，頂部呈現方案 A 藍色質感提示橫幅（`👁️ 唯讀檢視模式：手機端此檢視僅供瀏覽資訊；若需修改或指派任務，請前往【清單】頁面編輯。`），並配置 `[📋 前往清單編輯]` 快捷按鈕，點擊可即時切換至清單檢視並轉為可編輯狀態。
- **異動模組**：`apps/web/src/pages/Week.tsx`, `apps/web/src/components/TaskDrawer.tsx`, `apps/web/src/App.tsx`。

### <a id="cr-174"></a>CR-174 (2026-08-20) — 行動端互動策略：手機版全站外層列表純展示，點擊直達任務修改頁（TaskDrawer）專職編輯

- **使用者需求**：手機版只在任務修改頁面裡面改東西，其他畫面都只是拿來顯示。
- **互動重構實作**：
  1. **手機點擊直通任務修改抽屜 (`App.tsx`)**：
     - 在手機端（螢幕寬度 `< 768px`），使用者點擊任何任務卡片、側欄項目或列表項目，一律直接呼叫 `setOpenTask(id)` 開啟任務修改抽屜（`TaskDrawer`）。
  2. **外層視圖純淨顯示化 (`List.tsx`, `Board.tsx`)**：
     - 外層列表與看板在手機端專注於清爽直觀之單欄資訊呈現（標題、狀態、負責人、起訖日、進度與警示），去除繁雜的行內快速改動下拉選單與拖曳誤觸，修改操作統一收斂至專用之任務詳情抽屜。
- **異動模組**：`apps/web/src/App.tsx`, `apps/web/src/pages/List.tsx`, `apps/web/src/pages/Board.tsx`。

### <a id="cr-173"></a>CR-173 (2026-08-20) — 行動端排版重構：手機畫面全站滿版垂直單欄呈現（徹底消除橫向滑動與側欄推擠）

- **使用者需求**：手機畫面不要往右滑動，改為一格一格全部顯示；左側選單收縮時右側內容直接滿版。
- **重構與響應式實作**：
  1. **清單視圖手機單欄滿版卡片流 (`List.tsx`)**：
     - 手機端 (`md:hidden`) 淘汰大寬度表格與橫向滾動條，全面改為 100% 滿版垂直卡片流。
     - 每張卡片完整收納 Ref 編號、標題、狀態色塊、負責人頭像、起訖時間、進度條與各類警示徽章，點擊直接開啟詳情，零橫向溢出。
  2. **看板視圖手機狀態頁籤切換器 (`Board.tsx`)**：
     - 手機端提供頂部狀態切換頁籤（待辦／進行中／待驗收／已完成…），點選即滿版垂直瀏覽該狀態卡片，告別橫向跨欄滑動。
  3. **側欄行動抽屜與全域容器滿版適配 (`EpicSidebar.tsx`, `App.tsx`)**：
     - 側欄在手機端展開時以抽屜式浮層（Drawer Overlay）與半透明背景遮罩呈現，收合時右側內容立即佔滿 100% 螢幕寬度。
     - 全域加上 `w-full max-w-full overflow-x-hidden`，徹底杜絕手機畫面水平晃動或右滑溢出。
- **異動模組**：`apps/web/src/pages/List.tsx`, `apps/web/src/pages/Board.tsx`, `apps/web/src/components/EpicSidebar.tsx`, `apps/web/src/App.tsx`。

### <a id="cr-172"></a>CR-172 (2026-08-20) — 行動裝置自適應：針對複雜頁面新增「建議使用電腦版觀看」友善提示與手機側欄自動收合

- **使用者需求**：檢查所有頁面手機自適應，若畫面不適合手機操作，提示使用者使用電腦版觀看。
- **檢查與適配實作**：
  1. **全站頁面適配診斷**：
     - **手機友好視圖**：清單、看板、月曆、週檢視、儀表板、對外詢問、成員頁、回收站、任務抽屜（均具備彈性寬度、單欄堆疊或水平滾動）。
     - **電腦版建議視圖**：任務關聯圖 (`TaskGraph`)、系統流程圖 (`SystemFlow`)、甘特圖 (`Gantt`)、各語法範例 (`Playground`)（具備高精準畫布拖曳、多向雙向拉線或橫向跨月甘特圖排程）。
  2. **電腦版建議提示元件 (`DesktopRecommendedNotice.tsx`)**：
     - 當螢幕寬度 `< 768px` 進入上述 4 個視圖時，展示 `💻 建議使用電腦版觀看` 友善提示面板。
     - 提供「📋 切換至清單檢視（推薦）」快速跳轉與「繼續在手機上瀏覽」彈性選項（關閉後頂部保留小提示條）。
  3. **手機端側欄自動收合 (`EpicSidebar.tsx`)**：
     - 窄螢幕預設自動收合為窄條，避免佔據手機主要內容視野。
- **異動模組**：`apps/web/src/components/DesktopRecommendedNotice.tsx`, `apps/web/src/App.tsx`, `apps/web/src/components/EpicSidebar.tsx`。
