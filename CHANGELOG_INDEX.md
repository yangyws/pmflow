...
- **變更檔案**:
  - [`apps/api/src/routes/tasks.ts`](file:///D:/github/pmflow/apps/api/src/routes/tasks.ts):
    1. **問題單建立日與截止日異動權限保護 (`CR-242`)**：在 `PATCH /tasks/:id` 與 `POST /tasks/:id/reschedule` 路由中加入檢查，問題單（`BUG`）之建立日期（`startDate`）全域不可異動；截止日期（`dueDate`）僅允許原建立者（含職務代理人）或管理者進行調整，非建立者修改將拋出 403 禁止。
    2. **問題單換欄關閉權限保護與進度連動 (`CR-242`)**：在 `POST /tasks/:id/move` 路由中，問題單換欄移至完成欄位時同樣限制僅原建立者或管理者可執行，並自動連動進度為 100%；移回未完成欄位時自動重置進度為 0%。
...
- **變更檔案**:
  ...
  - [`apps/web/src/components/TaskDrawer.tsx`](file:///D:/github/pmflow/apps/web/src/components/TaskDrawer.tsx):
    1. **詳情抽屜狀態選單限制與按鈕簡化 (`CR-240`)**：問題單狀態下拉選單僅呈現「未解決」（紅色標籤）與「已解決」（綠色標籤），標籤顯示為「處理狀態」，非原建立者/管理者無法直接選「已解決」；移除額外的關閉與重置按鈕，統一直接透過狀態下拉選單進行解決與重置為未解決。
    2. **問題單精簡欄位 (`CR-240`)**：針對問題單（`type: BUG`），隱藏「優先級」與「排程模式」，並擴充「負責人」欄位寬度，使問題單資訊更加清爽專注。
...
- **變更檔案**:
  ...
  - [`docs/CHANGELOG.md`](file:///D:/github/pmflow/docs/CHANGELOG.md): 記錄 `CR-185` 條目與細節。
  - [`docs/NEXT-SESSION.md`](file:///D:/github/pmflow/docs/NEXT-SESSION.md): 更新進度至 `CR-185`。

### Previous Changes: Graph Box Capacity Hint Relocation (CR-184)
- **變更檔案**:
  - [`TaskGraph.tsx`](file:///D:/github/pmflow/apps/web/src/pages/TaskGraph.tsx):
