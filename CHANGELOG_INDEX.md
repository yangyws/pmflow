...
- **變更檔案**:
  ...
  - [`TaskDrawer.tsx`](file:///D:/NewProject/pmflow-git/apps/web/src/components/TaskDrawer.tsx): 提供「事件內容」編輯區塊、新增「問題標題」欄位支援一鍵開立「問題單」並自動建立左右關聯線（問題單獨立不進入子任務上下階層）、移除關聯類型與間隔設定精簡關聯建立、折疊收納「📜 歷史已解決問題 (N 件)」，並將按鈕文字更新為「刪除事件」。
