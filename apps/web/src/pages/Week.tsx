      ...
      <div className="min-h-0 flex-1 overflow-auto p-3 sm:p-4">
        {groups.length === 0 ? (
          ...
          <>
                  ...
                  <section key={g.key} className="space-y-2">
                    ...
                    {!off && g.rows.map(r => (
                      <MobileTaskCard
                        ...
                        row={r}
                        onOpen={onOpen}
                        typeName={typeOf(r.task.type)}
                        typeColor={typeColorOf(r.task.type)}
                        status={statuses.find(s => s.key === r.task.statusKey)}
            ...
            {/* 🖥️ 電腦版詳細表格（水平展開） */}
            <div className="hidden md:block min-w-[52rem] overflow-hidden rounded-lg bg-white ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-700">
...
function MobileTaskCard({
  row, onOpen, typeName, typeColor, status, isFocused,
}: {
  row: Row
  onOpen: (taskId: string) => void
  typeName: string
      ...
      <div className="mt-2 flex items-center justify-between text-[11px] text-slate-500 dark:text-slate-400">
        <div className="flex items-center gap-1.5 min-w-0">
          {t.assigneeName ? (
            ...
            <span className="text-slate-400">{W.unassigned}</span>
          )}
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <span>{dateText(t)}</span>
          {t.type !== 'BUG' && (
